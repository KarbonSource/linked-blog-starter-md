...
The easiest way to measure the mass of an object is to measure measure its weight, often by comparing with a standard equation

Mass plays two rather different roles in mechanics. The weight of an object is proportional to its mass as stated in the equation w = mg; we call the property related to gravitational interactions gravitational mass. On the other hand, we call the inertial property that appears in Newton's second law $\Sigma{\overrightarrow{F}}$ = m$\overrightarrow{a}$ the inertial mass. If these two quantities were different, the acceleration due to gravity might well be different.

# 4.5 Newton's Third Law
A force acting on an object is always the result of its interaction with another object, so forces always com in pairs.
![[Pasted image 20250207081851.png]]
![[Pasted image 20250207081917.png]]

In the statement of Newton's third law, "action" and "reaction" are the two opposite forces; we sometimes refer to the as an action-reaction pair. We often say simply that the forces are "equal and opposite" meaning that they have equal magnitudes and opposite directions.
